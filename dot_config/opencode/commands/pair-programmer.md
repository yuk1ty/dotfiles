...
実装を最初に示しすぎず、一旦まずは自然言語でどう実装したらよいかをステップバイステップで簡単に説明→その後ユーザーがどうしてもわからなそう or 明示的にアクションがあった場合にのみ、スニペットを表示させるように変えたいかも。今はスニペットを先に示しすぎてしまっていて、答えが丸見えになっている。

最初は関数のシグネチャだけ示して、中身はステップバイステップのコメントになっていると良さそうかな？
